({
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") 
            {
                ...
                {                    
                    ...
                    {
                        if(integrationMappingList[i].Mapping_Contract__c == component.get("v.mappingContractDetails.Name"))
                        {                            
    ...
    {
        var action = component.get("c.getMappingContractDetailsList");
        action.setParams({            
            "objectName" : 'Mapping_Contract__c',
            "recId" : component.get("v.recordId")
        });  
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") 
            {
                ...
                component.set("v.ownerDetails", resultList[0][0]);
                component.set("v.mappingContractDetail",resultList[1][0]);                
                if(resultList[2].length >  0)
                {                    
                    component.set("v.isAvailableContractRules",true);
                }
                else{
                    component.set("v.isAvailableContractRules",false);
                }
                var sourceDatastoreDataList = resultList[3];
                ...
                component.set("v.allMappingContractDetailsList" , resultList[5]);
                
                component.set("v.timeZone",resultList[6][0].TimeZoneSidKey);
                var namedCredentialDataList=resultList[7];
                ...
                var picklistValuesMap = JSON.parse(resultList[8][0].Description__c);
                component.set("v.sourceObjectOptionsMap",picklistValuesMap);
                component.set("v.sourceObjectOptions", component.get("v.sourceObjectOptionsMap")[resultList[1][0].Source_Datastore__c]);
    ...
    handleValidateMappingContract:function (component, event,helper) 
    {
        ...
        if(errorFieldsSet.size>0)
        {
            ...
            //alert('These required fields must be completed: ' + errorFieldsStr.substring(0,errorFieldsStr.length - 2));
        ...
        //alert('1 errorFieldsSet.size ::: ' + errorFieldsSet.size)
        ...
        if(!isSalesforceSelected)
        {
            if(!helper.urlvalidation(component, event, helper,mappingContractDetail.End_point_URL__c))
            {
                ...
                helper.handleValidateMappingContractHelper(component,event,helper,erroMessage,true);
                //alert('mappingContractEndPointURLError ::: ', component.get("v.mappingContractEndPointURLError"));
                return component.get("v.isErrorFound");
            }
            /*else
            {
        ...
        if(urlValid){
            ...
            for(var i = 0 ; i < allMappingContractDetailsList.length ; i++)
            {
                   ...
                   && mappingContractDetail.Id != allMappingContractDetailsList[i].Id)
                {
                    ...
                    duplicateName = true;
                    component.set("v.mappingContractNameError" , " slds-has-error ");
                    //alert('mappingContractNameError ::: ', component.get("v.mappingContractNameError"))
                    component.set("v.isErrorFound","true");
    ...
    saveMappingContract : function(component,event,helper){
        ...
        action.setCallback(this, function(a) {
            ...
            if (state === "SUCCESS") {                 
                helper.LightningToastmessage(component, event, helper,"Mapping Contract updated successfully.","Success");                
    ...
    urlvalidation : function(component,event,helper,msg){
        var validity = component.find("urlFieldID").get("v.validity");
